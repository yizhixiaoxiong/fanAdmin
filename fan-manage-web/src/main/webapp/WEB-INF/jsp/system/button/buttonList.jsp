		...
		<div class="main-content-inner">
			<div class="page-content">
				<div class="row">
					...
					<!-- 检索  -->
					<form action="fhbutton/list" method="post" name="Form" id="Form">
...
$(function(){
	...
	$('#simple-table > thead > tr > th input[type=checkbox]').eq(0).on('click',function(){
		var th_checked = this.checked;	//thead中th的元素，也就是上述选择器的元素
...
function add(){
	...
	diag.Drag=true;
	diag.Title = "新增";
	diag.URL = '<%=basePath%>rest/fhbutton/goAdd';
 	diag.Width = 450;
 	diag.Height = 358;
	diag.CancelEvent = function(){ //关闭事件
		window.location.href="rest/fhbutton/list"
		diag.close();
	 };
	 diag.show();
}
...
function edit(Id){
	 ...
	 diag.Drag=true;
	 diag.Title ="编辑";
	 diag.URL = '<%=basePath%>rest/fhbutton/goEdit.do?FHBUTTON_ID='+Id;
	 diag.Width = 450;
	 diag.Height = 358;
	 diag.CancelEvent = function(){ //关闭事件
		diag.close();
...
function del(Id){
	bootbox.confirm("确定要删除吗?",function(result){
		if(result){
			top.jzts();
			var url = "<%=basePath%>rest/fhbutton/delete?FHBUTTON_ID="+Id+"&tm="+new Date().getTime();
		...
		if(result){
			...
			if(str == ''){
				...
				if(msg == '确定要删除选中的数据吗?'){
					...
					$.ajax({
						type:"POST",
						url: '<%=basePath%>rest/fhbutton/deleteAll?tm='+new Date().getTime(),
				    	data: {DATA_IDS:str},
				    	dataType:'json',
				    	cache:false,
