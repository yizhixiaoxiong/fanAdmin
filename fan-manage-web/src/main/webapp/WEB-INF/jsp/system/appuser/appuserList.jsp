			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
						<!-- 检索  -->
						<form action="rest/happuser/listUsers" method="post" name="userForm" id="userForm">
...
function add(){
	...
	diag.Drag=true;
	diag.Title = "新增会员";
	diag.URL = '<%=basePath%>rest/happuser/goAddU';
	diag.Width = 600;
	diag.Height = 415;
...
function editUser(user_id){
	 ...
	 diag.Drag=true;
	 diag.Title ="会员资料";
	 diag.URL = '<%=basePath%>rest/happuser/toEdit?USER_ID='+user_id;
	 diag.Width = 600;
	 diag.Height = 415;
	 diag.CancelEvent = function(){ //关闭事件
		 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
...
function delUser(userId,username){
	//提示框
	bootbox.confirm("确认删除["+username+"]?",function(result){
		if(result){
			top.jzts();
			var url = "<%=basePath%>rest/happuser/deleteU?USER_ID="+userId+"&tm="+new Date().getTime();
...
function makeAll(msg){
	var str = "";
	var ids = document.getElementsByName("ids");	//获取集合
	...
	if(str == ""){	//未选择
		...
		if(msg == "确定要删除选中的数据吗?"){
			...
			$.ajax({
				type: "POST",
				url: '<%=basePath%>rest/happuser/deleteAllU?tm='+new Date().getTime(),
		    	data: {USER_IDS:str},
				dataType:'json',
				cache:false,
