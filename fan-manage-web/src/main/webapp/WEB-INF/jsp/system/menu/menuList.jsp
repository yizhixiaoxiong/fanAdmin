...
<body class="no-skin">
	...
	<script type="text/javascript">
		...
		function goSonmenu(MENU_ID){
			top.jzts();
			window.location.href = '<%=basePath%>rest/menu/list?MENU_ID='+MENU_ID;
		}
		//新增菜单(非弹出框模式)
		//参数：MENU_ID 为父id
		function addmenu(MENU_ID){
			top.jzts();
			window.location.href = '<%=basePath%>rest/menu/toAddmenu?PARENT_ID='+MENU_ID;
		}
		//编辑菜单
		//参数：MENU_ID 本菜单id
		function editmenu(MENU_ID){
			top.jzts();
			window.location.href = '<%=basePath%>rest/menu/toEditmenu?MENU_ID='+MENU_ID;
		}
		...
		function delmenu(MENU_ID){
			bootbox.confirm("确定要删除此菜单吗?", function(result) {
				if(result){
					var url = "<%=basePath%>rest/menu/delete?MENU_ID="+MENU_ID+"&guid="+new Date().getTime();
					top.jzts();
					$.get(url,function(data){
						data = JSON.parse(data);
						if("success" == data.result){
							parent.location.href = "<%=basePath%>rest/menu/listAllMenu?MENU_ID="+${MENU_ID};
		...
		function editTb(MENU_ID){
			...
			diag.Drag = true;
			diag.Title = "编辑图标";
			diag.URL = '<%=basePath%>rest/menu/toEditicon?MENU_ID'+MENU_ID;
			diag.Width = 800;
			diag.Height = 380;
