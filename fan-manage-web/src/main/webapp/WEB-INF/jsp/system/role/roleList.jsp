				...
				<div class="row">
					<div class="col-xs-12">
						...
						<table style="margin-top:8px;">
							<tr>
								<td style="width:69px;"><a href="javascrpt:addRole(0);" class="btn btn-sm btn-success">新增组</a></td>
								<c:choose>
									<c:when test="${not empty roleList}">
										<c:forEach items="${roleList}" var="role" varStatus="vs">
											<td style="width:100px;" class="center" 
												<c:choose>
													<c:when test="${pd.ROLE_ID == role.ROLE_ID}">bgcolor="#FFC926" onMouseOut="javascript:this.bgColor='#FFC926';"</c:when>
													<c:otherwise>bgcolor="#E5E5E5" onMouseOut="javascript:this.bgColor='#E5E5E5';"</c:otherwise>
												</c:choose>  
												onMouseMove="javascript:this.bgColor='#FFC926';" >
												<a href="rest/role?ROLE_ID=${role.ROLE_ID }" style="text-decoration:none; display:block;">
													<i class="menu-icon fa fa-users"></i>
													<font color="#666666">${role.ROLE_NAME }</font>
												</a>
											</td>
											<!-- 按钮之间的间隙 -->
						...
						<table>
							...
							<tr>
								...
								<td>
									<c:if test="${QX.edit == 1 }">
										<a class="btn btn-mini btn-info" onclick="editRole('${pd.ROLE_ID }');">修改组名称
											<i class="icon-arrow-right  icon-on-right"></i>
										</a>
									</c:if>
									<c:choose>
										<c:when test="${pd.ROLE_ID == '99'}"></c:when>
										<c:otherwise>
											<c:if test="${QX.edit == 1 }">
									...
									<c:choose> 
										<c:when test="${pd.ROLE_ID == '1' or pd.ROLE_ID == '2'}"></c:when>
										<c:otherwise>
										 <c:if test="${QX.del == 1 }">
										 	<a class='btn btn-mini btn-danger' title="删除" onclick="delRole('${pd.ROLE_ID }','z','${pd.ROLE_NAME }');">
...
function addRole(pid){
	...
	diag.Drag = true;
	diag.Title = "新增";
	diag.URL = '<%=basePath%>rest/role/toAdd?parent_id='+pid;
	diag.Width = 222;
	diag.Height = 100;
	diag.CancelEvent = function(){//关闭事件
		if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
			top.jzts();
			setTimeout("self.location.reload()",100); //当前窗口reload
		}
		diag.close();
	}
	diag.show();
...
function editRole(roleId){
	...
	diag.Drag = true;
	diag.Title = "修改组名";
	diag.URL = '<%=basePath%>rest/role/toEdit?ROLE_ID='+roleId;
	diag.Width = 222;
	diag.Height = 100;
	diag.CancelEvent = function(){//关闭事件
		if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
...
//删除
//参数：msg: 区分角色组或者角色。"z"表示组，"c"表示角色
function delRole(ROLE_ID,msg,ROLE_NAME){
	bootbox.confirm("确定要删除["+ROLE_NAME+"]吗?", function(result) {
		if(result){
			var url = "<%=basePath%>rest/role/delete?ROLE_ID="+ROLE_ID+"&guid="+new Date().getTime();
			top.jzts();
			$.getJSON(url,function(data){
				if("success" == data.result){
					if(msg == "c"){	//角色
						...
						top.jzts();
						window.location.href="rest/role";
					}
...
function editRights(ROLE_ID){
	 ...
	 diag.Drag = true;
	 diag.Title = "菜单权限";
	 diag.URL = '<%=basePath%>rest/role/menuqx?ROLE_ID='+ROLE_ID;
	 diag.Width = 320;
	 diag.Height = 450;
	 diag.CancelEvent = function(){ //关闭事件
			diag.close();
...
function roleButton(ROLE_ID,msg){
	...
	diag.Drag = true;
	diag.Title = Title;
	diag.URL = '<%=basePath%>rest/role/b4Button?ROLE_ID='+ROLE_ID+'&msg='+msg;
	diag.Width = 330;
	diag.Height = 450;
	diag.CancelEvent = function(){ //关闭事件
		diag.close();
