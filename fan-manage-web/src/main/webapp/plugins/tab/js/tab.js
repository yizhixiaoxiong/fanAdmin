...
function TabView(f) {
    ...
    //jQuery的对象插件，将f覆盖到g
    $.extend(g, f);
    //如果是bottom则返回bottom，否则返回""
    var b = g.position == "bottom" ? "_bottom" : "";
    this.id = g.cid;
    this.pid = g.pageid;
    this.tabs = null;			//新属性预计是tabs的集合
   ...
   /* $("#tab_menu").append(h.replace("{0}", this.id))); //tab1 */ //在tab_menu 元素结尾处后面添加h tab_menu在tab.jsp页面上
    /*现在的拼接情况是
     * <div id="tab_menu">
     * 	<div class="benma_ui_tab" id="tab1">
     *   	<div class="tab_hr">
     * 		</div>
     * 	</div>
     * </div>
    ...
    function e(n) {
        ...
        if (a.current == null || a.current != this) {//这里的this = n
